function [prec_ext,prec_flex,sens_ext,sens_flex,esp_ext,esp_flex,overall_acc]=assess_results(N,voluntario,cell_acel,cell_cmd_plot)
...
%        v_mov=[v_mov w_mov(i):w_rel(i)];%cria vetor com posições de cmd_plot onde deve haver movimento
        ...
        trecho_rel=[w_mov(i)+pos_mid w_rel(i)];
        if i<length(w_mov),
            trecho_rep=[w_rel(i) w_mov(i+1)];
        end
    ...
    for k=1:size(verdade_mov,1),
            ...
            if length(find_vp)>=floor((verdade_mov(k,2)-verdade_mov(k,1))/2),%considera-se verdadeiro positivo se mais da metade do trecho for verdadeiro positivo 
                num_vp=num_vp+1;
            else num_fn=num_fn+1;
                find_fp_2=find((cmd_plot(verdade_mov(k,1):verdade_mov(k,2)))==setdiff(vect_isinal,vect_isinal(isinal)));
                if length(find_fp_2)>=floor((verdade_mov(k,2)-verdade_mov(k,1))/2),%considera-se verdadeiro positivo se mais da metade do trecho for verdadeiro positivo 
                    num_fp_2=num_fp_2+1;
     ...
     for l=1:size(verdade_rep,1),
            ...
            if length(find_fp)>=floor((verdade_rep(l,2)-verdade_rep(l,1))/2),%considera-se verdadeiro positivo se mais da metade do trecho for verdadeiro positivo 
                num_fp=num_fp+1;
            else num_vn=num_vn+1;
                 find_fp_2=find((cmd_plot(verdade_rep(l,1):verdade_rep(l,2)))==setdiff(vect_isinal,vect_isinal(isinal)));
                    if length(find_fp_2)>floor((verdade_rep(l,2)-verdade_rep(l,1))/2),%considera-se verdadeiro positivo se mais da metade do trecho for verdadeiro positivo 
                        num_fp_2=num_fp_2+1;
                    else num_vn_2=num_vn_2+1;
    ...
    num_p_win(isinal)=num_p_win(isinal)+num_vp+num_fp
    num_p_win(setdiff(vect_isinal,vect_isinal(isinal)))=num_p_win(setdiff(vect_isinal,vect_isinal(isinal)))+num_fp_2
...
prec_ext=num_vp_win(1)/num_p_win(1);
sens_ext=num_vp_win(1)/num_v_win(1);
prec_flex=num_vp_win(2)/num_p_win(2);
sens_flex=num_vp_win(2)/num_v_win(2);
