...
for isinal=1:length(vect_isinal),
    ...
    for i=1:length(w_mov) 
        ...
        if i<length(w_mov),
            ...
            trecho_rel=[w_rel(i)+1 pos_final];
        end
    ...
    %%avaliação dos trechos de relaxamento
...
%             num_vn_rel=num_vn_rel+verdade_rel(l,2)-verdade_rep(l,1)+1-length(find_fp_rel);   
     ...
     for m=1:size(verdade_rep,1),
            find_vp_rep=find((cmd_plot(verdade_rep(m,1):verdade_rep(m,2)))==0);         
            num_vp_rep=num_vp_rep+length(find_vp_rep);    
            find_fp=find((cmd_plot(verdade_rep(m,1):verdade_rep(m,2)))==vect_isinal(isinal));
            num_fp=num_fp+length(find_fp);
            num_vn=num_vn+verdade_rep(m,2)-verdade_rep(m,1)+1-length(find_fp);
            find_fp_2=find((cmd_plot(verdade_rep(m,1):verdade_rep(m,2)))==setdiff(vect_isinal,vect_isinal(isinal)));
            num_fp_2=num_fp_2+length(find_fp_2);
            num_vn_2=num_vn_2+verdade_rep(m,2)-verdade_rep(m,1)+1-length(find_fp_2);   
            find_fp_rel=find((cmd_plot(verdade_rep(m,1):verdade_rep(m,2)))==-1);
            num_fp_rel=num_fp_rel+length(find_fp_rel);
            num_vn_rel=num_vn_rel+verdade_rep(m,2)-verdade_rep(m,1)+1-length(find_fp_rel);          
    end
    ...
    num_fp_win(setdiff(vect_isinal,vect_isinal(isinal)))=num_fp_win(setdiff(vect_isinal,vect_isinal(isinal)))+num_fp_2;
...
arq_resultado_xlsx='resultado_final_10.xlsx';
sheet=voluntario;
